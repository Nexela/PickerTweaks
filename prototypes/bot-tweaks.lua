...
local scale = settings.startup['picker-adjustable-bot-scale'].value / 100

local bot_animations = {'idle', 'idle_with_cargo', 'in_motion', 'in_motion_with_cargo', 'shadow_idle', 'shadow_in_motion', 'shadow_in_motion_with_cargo', 'working'}
...
for index, bot in pairs(types) do
    for _, entity in Data:pairs(data.raw[bot]) do
        ...
        if settings.startup['picker-adjustable-bot-scale'].value ~= 100 then
            --[[
                "name": "SmallRobots",
