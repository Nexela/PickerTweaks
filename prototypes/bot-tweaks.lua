...
for index, bot in pairs(types) do
    for _, entity in Data:pairs(data.raw[bot]) do
        ...
        if bot == 'construction-robot' then
            if settings.startup['picker-unminable-construction-robots'].value then entity.minable = nil end
        ...
        if bot == 'logistic-robot' then
            if settings.startup['picker-unminable-logistic-robots'].value then entity.minable = nil end
            if settings.startup['picker-noalt-logistic-robots'].value then flags:add('hide-alt-info') end
        end
